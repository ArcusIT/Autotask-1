...
Function Update-Functions {
  ...
  Process {
    ...
    Foreach ($Tenant in $Script:Cache.GetEnumerator()) { 
      ...
      If ($Tenant.Key -eq '00' -and $FunctionSet -eq 'Static') {
        $RootPath = '{0}\Nested' -F $MyInvocation.MyCommand.Module.ModuleBase
      }
      ...
      Else {
        # Do not create static functions pr tenant or dynamic functions for '00'
        Continue
      }
